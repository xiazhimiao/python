...
def tsp_backtracking(G, start, path, visited, current_distance, min_distance, min_path):
    if len(path) == len(G.nodes()):
        try:
            # 尝试回到起点
            return_distance = nx.dijkstra_path_length(G, path[-1], start, weight='weight')
    ...
    for neighbor in G.neighbors(path[-1]):
        if neighbor not in visited:
            try:
                edge_distance = nx.dijkstra_path_length(G, path[-1], neighbor, weight='weight')
                new_visited = visited.copy()
                new_visited.add(neighbor)
                tsp_backtracking(G, start, path + [neighbor], new_visited, current_distance + edge_distance,
                                 min_distance, min_path)
            except nx.NetworkXNoPath:
                continue


def find_shortest_tour(G, start_name):
    start_id = name_to_id(G, start_name)
    if start_id is None:
        print(f"未找到名为 {start_name} 的顶点。")
        return

    min_distance = [float('inf')]
    min_path = []
    visited = set([start_id])
    tsp_backtracking(G, start_id, [start_id], visited, 0, min_distance, min_path)

    if min_path:
        node_names = [G.nodes[node]['name'] for node in min_path]
        print(f"从 {start_name} 出发游览所有景点的最短路线: {' -> '.join(map(str, node_names))}")
        print(f"路线总长度: {min_distance[0]} 公里")
    else:
        print("未找到可行的环游路线。")
    ...
    if G is not None:
        while True:
            ...
            if choice == '1':
                ...
                start_name = input("请输入起始游览的景点名称: ")
                find_shortest_tour(G, start_name)
            elif choice == '6':
                break
            else:
                print("无效的选择，请重新输入。")
